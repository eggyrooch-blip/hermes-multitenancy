...
16. **Group `@everyone` never triggers the bot.** Admission (`_admit`) ignores `@_all` in *any* reply mode — detected via structured mention metadata **or** raw `@_all` — so an `@所有人` broadcast can never wake every routed agent in a group.
