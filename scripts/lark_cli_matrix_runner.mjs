...
const CAPABILITIES = {
  ...
  contact: { skill: 'lark-contact', service: 'contact', schemaCommand: 'contact +search-user --help', canary: '搜索用户“孙可”。请只执行这一条 lark_cli 命令：`contact +search-user --query "孙可" --format json --as user`；不要改成 keyword，不要去掉 +。只回答是否找到、open_id 和姓名。' },
...
async function main() {
  if (arg('self-test')) {
    ...
    if (selfTest === 'prompt-canaries') {
      const expected = {
        contact: 'contact +search-user --query "孙可" --format json',
