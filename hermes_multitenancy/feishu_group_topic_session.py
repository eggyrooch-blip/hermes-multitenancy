...
def group_topic_epoch_actor(event: Any, sender_open_id: str) -> str:
    return group_topic_thread(event) or sender_open_id or "unknown"
...
async def _fetch_thread_id(adapter: Any, message_id: str, chat_id: str) -> Optional[str]:
    ...
    if item is None or str(getattr(item, "chat_id", "") or "") != chat_id:
        return None
    return str(getattr(item, "thread_id", "") or "").strip() or None
...
def _patch_process_inbound(adapter_type: Any) -> None:
    ...
    async def wrapped(self: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        canonical_thread_id = raw_thread_id

        if chat_type != "p2p" and not canonical_thread_id:
        ...
        if message_id:
            _put_bounded(
                _mapping(self, _PENDING_ATTR),
                message_id,
                (
                    canonical_thread_id,
                    raw_thread_id,
                    root_id,
                    bool(canonical_thread_id and chat_type != "p2p"),
                ),
            )
...
def _patch_dispatch_inbound(adapter_type: Any) -> None:
    ...
    async def wrapped(self: Any, event: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        if metadata and source is not None:
            canonical_thread_id, raw_thread_id, root_id, is_topic = metadata
            if canonical_thread_id:
                source.thread_id = canonical_thread_id
            source.hermes_group_topic = is_topic
            source.hermes_raw_thread_id = raw_thread_id
            source.hermes_root_id = root_id
...
def _patch_batch_key(adapter_type: Any, method_name: str, flag: str) -> None:
    ...
    def wrapped(self: Any, event: Any, *args: Any, **kwargs: Any) -> str:
        ...
        if not is_group_topic_event(event):
            return key
        source = getattr(event, "source", None)
        sender = str(
