...
> 2026-06-24 implementation note: `agent-sharing-acl` implements a share ACL layer on top of owner-scoped agents. The new roles are `viewer`, `editor`, and `manager`. Viewer/editor grantees can run the shared agent but their WebUI session search/list scope is limited to rows whose `user_id` is the grantee open_id; managers can inspect all WebUI sessions for that shared agent and manage members. WebUI editor config writes are intentionally limited to chat-plane-safe sections; raw credentials, provider env, owner profile `.env`, and profile metadata are not shared.
