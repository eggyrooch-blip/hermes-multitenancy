...
| Threat | Status |
|---|---|
| Compromised skill calls `cat /Users/hermes/.ssh/id_rsa` | open — file-system reads outside the profile are still permitted by the kernel. Closes under档 B |
| Compromised skill opens a TCP socket to `192.0.2.249:443` (internal lateral) | open — no egress controls. Closes under档 B with the `network` profile |
...
conversation starts. The preflight only accepts local image markers that resolve
inside the routed profile's `workspace/uploads/` tree, normalizes common WebUI
paths such as `/workspace/uploads/...`, checks the filename extension, and
