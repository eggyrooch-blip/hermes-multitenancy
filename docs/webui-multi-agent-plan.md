...
> 状态：规划（未实现）。本文档是实现前的 SPEC，决策已与 sunke 对齐。
...
`router.py:_apply_lark_cli_profile_defaults` 原本把 feishu 平台 toolset 模式钉死成 `explicit`，导致 `_resolve_enabled_toolsets` 原样返回 `["lark-cli"]`、丢掉 `skills` 工具集——skill 文件软链进了 profile 但 agent 清单里看不到（即 sunke 报告的"技能里看不到 lark skills"）。
