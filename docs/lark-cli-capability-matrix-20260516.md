...
3. T3: `sunke` profile WebUI -> `lark_cli` using user identity when UAT exists.
4. T4: `sunke` Feishu private chat -> `lark_cli` using user identity when UAT exists.
...
| T3 sunke WebUI -> user `lark-cli` | 25 capabilities x 3 messages = 75 rows | 72 | 3 | 0 | `t3-webui-full-fixed-20260516.jsonl`, `t3-tail-fixed-20260516-040306.jsonl` |
| T4 sunke Feishu private chat -> user `lark-cli` | 25 capabilities x 3 messages = 75 rows | 72 | 3 | 0 | `t4-representative-fixed-mark-20260516-035617.jsonl`, `t4-full-batch-a-20260516-0448.jsonl`, `t4-full-batch-b-20260516-0455.jsonl`, `t4-full-batch-c-20260516-0504.jsonl`, cleanup rows |
...
| T4 sunke Feishu private chat -> user profile | Create a Feishu doc through `docs +create --api-version v2` | Created doc `UHPadVNMKot3mrx19cqc2Z0nnrc` |
| T4 sunke Feishu private chat -> user profile | Read today's agenda through `calendar +agenda` | Returned 3 events: daily standup reminder, LARKCLI smoke event, UAT test event |
| T4 sunke Feishu private chat -> user profile | Read my tasks through `task +get-my-tasks` | Returned 3 tasks |
| T4 sunke Feishu private chat -> user profile | Run standup workflow via calendar + task | Both calls succeeded: 3 calendar events and 3 tasks |
| T4 sunke Feishu private chat -> user profile | Create Sheets / Slides artifacts | Created `OJ08sDsilhSMZhtTgBac30Nvnxc` sheet and `KHIEspGBXlxhP9dz910cq5YSnQc` slide deck |
...
| T3 sunke WebUI | `drive/3` | Missing or unavailable drive/search scope for the tested command |
| T3 sunke WebUI | `mail/3` | User mailbox capability is not enabled/available for this account |
| T3 sunke WebUI | `shared/3` | Auth status is external-provider managed, so interactive auth status cannot enumerate user/bot state |
| T4 sunke Feishu private | `drive/3` | Missing `search:docs:read` scope for `drive +search` |
| T4 sunke Feishu private | `mail/3` | Feishu Mail reports `user not found`; mailbox is not enabled/bound for this account |
| T4 sunke Feishu private | `shared/3` | Auth status is external-provider managed, so interactive auth status cannot enumerate user/bot state |
...
1. Add/authorize `search:docs:read` if `drive +search` should work for `sunke` UAT.
2. Enable/bind Feishu Mail mailbox if `mail +triage` should work for `sunke`.
