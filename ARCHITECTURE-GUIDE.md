...
> 2026-05-15 本机新增 `lark-cli-authsidecar` 构建门槛，尚未发布生产：`scripts/build_lark_cli_authsidecar.sh` 从固定 `larksuite/cli` 源码执行 `go build -tags authsidecar`，默认产物为 `~/.hermes/bin/lark-cli-authsidecar`。本机真实构建产物 `/Users/kite/.hermes/bin/lark-cli-authsidecar` 的 `--version` 为 `v0.0.0-20260515081849-4a45e0013945`，命中 pinned source `4a45e00`，且用 `LARKSUITE_CLI_AUTH_PROXY` 探针确认不是 noauthsidecar build。
>
> 同日追加：`hermes_multitenancy/lark_cli_auth_broker.py` 已实现本机最小 credential-vault auth broker 核心和 localhost HTTP wrapper。它按官方 sidecar v1 canonical string 验签，限制 target host 只能是 `open.feishu.cn` / `open.larksuite.com`，限制 identity/auth-header，user identity 从 `multitenancy_credentials` 的 `profile_name + open_id + provider=feishu + secret_kind=uat` 读取 UAT 并只在转发请求时注入；`agent_real._build_subprocess_env()` 只给 AIAgent 子进程注入 `HERMES_LARK_CLI_BIN` 与 `LARKSUITE_CLI_*` sidecar plumbing，不转发 Feishu app secret 或 UAT 原文。随后 `_aiagent_subprocess_env_scope()` 已把 broker 生命周期接到每次 AIAgent 子进程 spawn：满足 sender open_id、`~/.hermes/bin/lark-cli-authsidecar` 存在、且能从 `HERMES_LARK_CLI_APP_ID` 或全局 vault app credential 行读到公开 app_id 时，为该 run 生成短期 HMAC key、启动 localhost broker、把 env 注入 child，并在 subprocess 结束/取消/失败后关闭。`lark-cli` 的实际执行点因此是 routed profile AIAgent 子进程；父进程只负责可信 auth broker，不能替 profile 直接跑 OAPI。本机仍未跑真实 Feishu/WebUI `lark-cli` canary；也未发布生产，不能宣称生产已通过 `lark-cli` 复用 Hermes UAT。
>
> `scripts/lark_cli_canary_preflight.py` / `hermes_multitenancy/lark_cli_canary.py` 提供 secret-free 前置检查：校验 authsidecar binary、credential vault key、全局 Feishu app credential、当前 profile/open_id UAT，并在 ready 时输出 `lark_cli` 的只读 `GET /open-apis/authen/v1/user_info` canary 参数；同脚本的 `import-legacy-uat` 可把已有 `~/.hermes/feishu_uat/<open_id>.json` 迁入 credential vault，`import-app-config` 可把 `~/.hermes/config.yaml` 中的 Feishu app credential 迁入全局 vault 行，输出只包含 open_id、app_id、scope 数和过期时间等非 secret 信息。本机加载 `/Users/kite/.hermes/.env` 后，已把全局 app credential 与 active profile `feishu_ee966643 / ou_63348b2d4c8f885768bdc6c7d7fc26ee` 的 UAT 导入 vault；第一次 `lark-cli` user_info canary 返回 Feishu `20005 invalid access token`，随后用 refresh token 刷新该 UAT 并回写 vault。真实直连 canary 已通过：`lark_cli -> lark-cli-authsidecar -> localhost auth broker -> Feishu GET /open-apis/authen/v1/user_info` 返回 `ok=true / exit_code=0 / code=0`，结果只输出字段存在性且未包含 token/app secret。期间发现 broker 直接转发 upstream `Transfer-Encoding: chunked` 会导致 sidecar 报 `invalid CR in chunked line`，已在 HTTP wrapper 中过滤 hop-by-hop/Content-Length 响应头并由本地 server 自行设置 `Content-Length`。2026-05-15 19:00 追加修复 AIAgent subprocess file-path loader：`aiagent_subprocess.py` 被直接执行时 `sys.path[0]` 是包目录，不是 repo root，导致 `_load_run_with_aiagent()` fallback 以非 package 名载入 `agent_real.py`，再因 relative import `lark_cli_auth_broker` 失败而退回 legacy stream；现在先插入 repo root 并直接 import `hermes_multitenancy.agent_real`。本机回归覆盖该场景，运行副本重启后 19:03 后无新的 subprocess import fallback。仍未发布生产。
>
...
>
> lark-cli 升级边界：multitenancy 只负责 profile runtime、authsidecar broker、UAT/bot token 注入和 profile skill 分发，不复制官方 OAPI 能力。升级官方 `lark-cli` 时先在本机更新 shared 官方 skills（`~/.hermes/skills/lark-*`），由 `profile-skill-defaults.yaml` 让 profile `skills/lark-*` 指向 shared symlink；再按 `scripts/build_lark_cli_authsidecar.sh` 构建新的 `~/.hermes/bin/lark-cli-authsidecar`。验证顺序是 `scripts/lark_cli_canary_preflight.py`、个人 profile 的 routed `lark_cli GET /open-apis/authen/v1/user_info` user canary、群 profile 的 bot canary；只有通过后才发布生产并重启 gateway。
...
- `profile-skill-defaults.yaml` 存在时，org sync/auto-provision 会从 shared `skills/` 分发默认 skill 到 profile `skills/`。业务 skill 使用复制并跳过 secret-looking 文件；官方顶层 `lark-*` skill 使用 symlink 指向 shared，避免升级后 profile 副本漂移。
