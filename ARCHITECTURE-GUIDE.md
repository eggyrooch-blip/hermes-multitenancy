...
>
> 2026-05-15 23:25 追加：官方 `lark-cli` 支持 `--as user|bot`，不是只能用用户身份。multitenancy broker 现在默认仍让 `LARKSUITE_CLI_DEFAULT_AS=user`，但 `LARKSUITE_CLI_STRICT_MODE=off`，显式 `identity=bot` 时允许 authsidecar 请求 bot identity；若全局 app credential 只有 `app_id/app_secret`，broker 会在父进程内向 `/open-apis/auth/v3/tenant_access_token/internal` 换取 tenant token，再注入给 lark-cli，不把 app_secret 下发到 AIAgent。测试覆盖 bot identity 从 app credential mint TAT、per-run broker 允许 `{"user","bot"}`，回归 `73 passed, 6 skipped`。产品边界：user identity 仍用于访问/创建用户私有资源；bot/app identity 用于机器人、租户级或应用可见资源，不能替代用户授权访问用户私有云文档/日历。
>
...
>
> lark-cli 升级边界：multitenancy 只负责 profile runtime、authsidecar broker 与 UAT 注入，不复制官方 OAPI 能力。升级官方 `lark-cli` 时先在本机更新 profile sandbox skills，再按 `scripts/build_lark_cli_authsidecar.sh` 构建新的 `~/.hermes/bin/lark-cli-authsidecar`，跑 `scripts/lark_cli_canary_preflight.py` 与 routed `lark_cli GET /open-apis/authen/v1/user_info` canary；只有通过后才发布生产并重启 gateway。
