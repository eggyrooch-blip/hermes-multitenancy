...
已知 gotcha：自定义聊天模型 selector 可能带 `[1m]` 这类仅用于展示的上下文标签；若把后缀原样作为上游模型 ID，LiteLLM 会拒绝本来有效的模型，因此聊天传输只剥离尾部数字 `k`/`m` 标签，共享媒体解析保持不变。

已知 gotcha：Hermes core 可能为空助手信封生成 `[System: Empty message content sanitised to satisfy protocol]`；共享助手事件边界必须在会话镜像和用户投递前移除这个精确占位符，包括它跨流分片出现时。
