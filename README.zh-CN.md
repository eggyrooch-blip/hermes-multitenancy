...
5. **默认 skill 与群/组织凭证从运行态分发。** `profile-skill-defaults.yaml`、`skill-distribution.yaml`、`skill-bundles.yaml` 描述托管 skill；sync 安装到 profile 时会跳过 secret-looking 文件。shared 顶层只要存在 `lark-*` skill，也会作为 managed symlink 自动安装到每个 profile，即使没有写进 distribution YAML，因为这批 skill 是官方 `lark-cli` 的使用说明层。`credential-materialization.yaml` 把 vault 里的加密 payload 写成 profile-local 兼容文件, 例如 `workspace/credentials/gitlab.token`；`profiles: ["*"]` 会展开为 active routing rows。entry 也可以声明 `env: GITLAB_TOKEN`, AIAgent 会从 vault 注入 env 并注册 terminal/code passthrough, 不需要模型读取 token 文件。
