...
def test_upsert_group_creates_row(table):
    ...
    assert row.display_label == "sunke-IT组"
...
def test_group_lookup_does_not_see_user_row(table):
    table.upsert(user_id="sunke", profile_name="kite", open_id="ou_sunke")
    assert table.lookup_by_chat_id("ou_sunke") is None
    assert table.lookup_by_chat_id("sunke") is None
...
def test_two_groups_same_owner_get_distinct_rows(table):
    ...
    table.upsert_group(
        ...
        owner_open_id="ou_inviter",
        display_label="sunke-财务组",
    )
    assert table.count_active(kind="group") == 2
    assert table.lookup_by_chat_id("oc_one").display_label == "sunke-IT组"
    assert table.lookup_by_chat_id("oc_two").display_label == "sunke-财务组"
...
def test_update_display_label(table):
    ...
    assert table.update_display_label("oc_x", "sunke-技术中台") is True
    row_after = table.lookup_by_chat_id("oc_x")
    assert row_after.display_label == "sunke-技术中台"
...
def test_list_by_owner_returns_only_owned_groups(table):
    """Web UI ACL — sunke只看到自己 owner 的群."""
    table.upsert_group(
        chat_id="oc_a",
        profile_name="g_a",
        owner_open_id="ou_sunke",
        display_label="sunke-A",
    )
    table.upsert_group(
        chat_id="oc_b",
        profile_name="g_b",
        owner_open_id="ou_sunke",
        display_label="sunke-B",
    )
    table.upsert_group(
        chat_id="oc_c",
        profile_name="g_c",
...
def test_migration_from_legacy_schema_preserves_user_rows(tmp_path):
    ...
    legacy_conn.executescript(
        ...
        VALUES
            ('sunke', 'kite', 'ou_sunke', 1, 1700000000, 1, 1700000000, 1700000000),
    ...
    t = RoutingTable(db)
    try:
        sunke = t.lookup_by_open_id("ou_sunke")
        alice = t.lookup_by_open_id("ou_alice")
        assert sunke is not None and sunke.kind == "user"
